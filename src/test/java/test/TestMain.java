...
//		String area = "전체";
//		String cate = "전체";
//		List<Store> cateList = storeMapper.appCateSearch(area, cate);
//		int i = 0;
//		for (Store s : cateList) {
//			i++;
//			System.out.println(i + "/ " + s.getSh_name() + " / " + s.getInduty_code_se_name());
//		}
//	}
	...
	// List<Review> items = rvmapper.select(9015);
	// if (items == null)
	// System.out.println("값이 없다");
	// for (Review item : items) {
	// System.out.println(item.getMname()+" / "+item.getMphoto()+" /
	// "+item.getRcontent()+" / "+item.getRelike());
...
// System.out.println(item.getMkey()+" / "+item.getSh_id()+" /
// "+item.getRkey());
// }
